...
const Video = () => {
  ...
  const startWebCam = () => {
    if (!!!navigator.getUserMedia) return alert("Webcam stream not supported");
    ...
    <div className="main">
      ...
      {previewVideo ? (
        ...
        <div className={`${styles.video} ${styles.beforeCapture}`}>
          <button
            className={`${styles.btnStart} bg-dark`}
            onClick={startWebCam}
          >
            start recording
          </button>
    ...
    <div className={styles.videoContainer}>
      <div className="text-center mt-4">
        <video
          ...
          autoPlay
          muted
          controls={recordedPlaying}
          className={styles.videoPlayer}
        ></video>
      </div>
      <div className="text-center">
        <button onClick={restartRecording}>Restart Recording</button>
        <button disabled={!!recordedVideo} onClick={stopRecording}>
          Stop Recording
        </button>
        <button disabled={!!!recordedVideo} onClick={playRecording}>
          Preview Recording
        </button>
      </div>
    </div>
